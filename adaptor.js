...
// 'use strict';
const oas = require("./lib/oas/oas.classes");
...
function convertArray(arr) {
    ...
    if (arr.length) {
        ...
        arr.isEmpty = true;
    arr.toString = function () { if (arrayMode === 'length')
            ...
            for (let s in ss) {
                ...
                if (scheme.type === 'http') {
                    ...
                    if (scheme.flows) {
                        ...
                        let flow = scheme.flows[Object.keys(scheme.flows)[0]];
...
function convertOperation(op, verb, path, pathItem, obj, api) {
    ...
    if (obj.httpMethodCase === 'original')
        operation.httpMethod = verb; // extension
    operation.path = path;
    ...
    effParameters = effParameters.filter((param, index, self) => self.findIndex((p) => {
    ...
    for (let pa in effParameters) {
        ...
        if (param instanceof oas.ParameterWithSchemaWithExampleInPath ||
            ...
            for (let p of schemaProperties) {
                if (typeof param.schema[p] !== 'undefined')
                    parameter[p] = param.schema[p];
            }
    ...
    if (op.requestBody) {
        ...
        if (op.requestBody instanceof oas.RequestBody) {
            ...
            if (op.requestBody.content) {
                let contentType = op.requestBody.content[Object.keys(op.requestBody.content)[0]];
                ...
                for (let p in schemaProperties) {
                    if (typeof contentType.schema[p] !== 'undefined')
                        operation.bodyParam[p] = contentType.schema[p];
                }
    ...
    for (let r in op.responses) {
        ...
        if (response.content) {
            ...
            let contentType = response.content[Object.keys(response.content)[0]];
            let mt = {};
...
function transform(api, defaults, callback) {
    ...
    if (typeMaps[lang])
        typeMap = typeMaps[lang];
    if (reservedWords[lang])
        reserved = reservedWords[lang];
