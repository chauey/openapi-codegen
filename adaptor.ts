            ...
            for (let s in ss) {
                ...
                if (scheme.type === 'http') {
                    ...
                    if (scheme.flows) {
                        ...
                        let flow: any = scheme.flows[Object.keys(scheme.flows)[0]];
...
function convertOperation(op: oas.Operation, verb, path, pathItem, obj, api) {
    ...
    for (let r in op.responses) {
        ...
        if (response.content) {
            ...
            let contentType: any = response.content[Object.keys(response.content)[0]];
            let mt: any = {};
            ...
            if (contentType && contentType.examples) {
                for (let example in contentType.examples) {
                    if ((<any>example).value) {
                        entry.hasExamples = true;
                        if (!entry.examples) entry.examples = [];
                        entry.examples.push({ contentType: mt.mediaType, example: JSON.stringify((<any>example).value, null, 2) });
                    }
                }
            }
